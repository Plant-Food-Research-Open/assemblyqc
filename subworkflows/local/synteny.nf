...
workflow SYNTENY {
    ...
    main:
    ch_between_target_asm_combinations  = !params.synteny.between_target_asm
                                        ? Channel.empty()
                                        : tuple_of_tag_fasta_seq_list
                                        | map { [it] }
                                        | collect
    ...
    ch_xref_fa_branch                   = tuple_of_tag_xref_fasta_seq_list
                                        | map { tag, fa, seq_list ->
                                            [ [ id: tag ], fa ]
                                        }
                                        | branch { meta, fa ->
    ...
    ch_xref_ungz_fasta_seq_list         = GUNZIP_FASTA.out.gunzip
                                        | mix(
    ...
    // MODULE: FILTER_SORT_FASTA_AND_VALIDATE_SEQ_LISTS | MUMMER & GET_FASTA_LEN
    ch_seq_lists                        = ch_between_target_asm_combinations
                                        .mix(
                                            tuple_of_tag_fasta_seq_list
                                            | combine(
                                                ch_xref_ungz_fasta_seq_list
                                            )
                                        )
                                        .tap { ch_full_tap_from_all_combinations }
                                        | map { target_tag, target_fa, target_sl, xref_tag, xref_fa, xref_sl ->
                                            [ "${target_tag}.on.${xref_tag}", target_sl, xref_sl ]
                                        }

    FILTER_SORT_FASTA_AND_VALIDATE_SEQ_LISTS ( ch_full_tap_from_all_combinations )
    | ( MUMMER & GET_FASTA_LEN )


    ch_dnadiff_inputs                   = FILTER_SORT_FASTA_AND_VALIDATE_SEQ_LISTS.out.tags_fasta_files
    ...
    // MODULE: DNADIFF | CIRCOS_BUNDLE_LINKS | ADD_COLOUR_TO_BUNDLE_LINKS
    DNADIFF ( ch_dnadiff_inputs )
    | CIRCOS_BUNDLE_LINKS
    | ADD_COLOUR_TO_BUNDLE_LINKS

    ch_coloured_links                   = ADD_COLOUR_TO_BUNDLE_LINKS.out.coloured_bundle_links

    ch_relabel_links_inputs             = ch_coloured_links
                                        | join(ch_seq_lists)

    // MODULE: RELABEL_BUNDLE_LINKS | SPLIT_BUNDLE_FILE_BY_TARGET_SEQS
    RELABEL_BUNDLE_LINKS ( ch_relabel_links_inputs )
    | SPLIT_BUNDLE_FILE_BY_TARGET_SEQS

    ch_circos_split_bundle_links        = SPLIT_BUNDLE_FILE_BY_TARGET_SEQS.out.split_file
                                        | map { flattenSplitBundles(it) }
                                        | flatten
                                        | buffer(size:3)

    // MODULE: RELABEL_FASTA_LEN
    ch_relabel_fasta_len_inputs         = GET_FASTA_LEN.out.tag_len_files
                                        | join(ch_seq_lists)

    RELABEL_FASTA_LEN ( ch_relabel_fasta_len_inputs )

    // MODULE: GENERATE_KARYOTYPE
    ch_generate_karyotype_inputs        = RELABEL_FASTA_LEN.out.relabeled_seq_lengths
                                        | cross(
    ...
    GENERATE_KARYOTYPE ( ch_generate_karyotype_inputs )

    // MODULE: CIRCOS
    ch_circos_inputs                    = GENERATE_KARYOTYPE.out.karyotype
                                        | join(
                                            ch_circos_split_bundle_links
    ...
    ch_versions                         = Channel.empty()
                                        | mix(MUMMER.out.versions.first())
                                        | mix(CIRCOS.out.versions.first())
