    ...
    with open(file_path) as tsv_file:
        for line in tsv_file:
            ...
            if orig_id in dictionary.keys():
                raise ValueError(f"{orig_id} is repeated in {file_path}. Each sequence ID should be unique")

            if new_id in dictionary.values():
                raise ValueError(f"{new_id} is repeated in {file_path}. Each sequence label should be unique")
...
def write_fasta_with_new_ids(fasta_file_path, orig_to_new_id_map, file_prefix):
    ...
    missing_ids = [orig_id for (orig_id, new_id) in orig_to_new_id_map.items() if new_id not in selected_ids]

    if len(missing_ids) > 0:
        raise ValueError(f"Some sequences {missing_ids} are missing from the fasta {fasta_file_path}")
